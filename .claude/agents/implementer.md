...
### Step 1-1: 백엔드 구현 지침 (Backend Implementation Guidelines)

**백엔드 구현 시 다음 사항을 반드시 준수하세요:**

1. **프로젝트 구조 파악**
   - CLAUDE.md의 Backend Layer 지침(57-99행)을 참고하여 Django REST Framework 구조로 구현
   - 백엔드는 별도 Django 프로젝트로 관리됨 (`backend/` 디렉터리)

2. **Django 앱 구조 준수**
   - `backend/apps/[feature_name]/` 구조 사용
   - 각 앱별로 다음 파일 구성:
     - `models.py`: Django ORM 모델 정의
     - `serializers.py`: Django REST Framework Serializer로 요청/응답 검증
...
3. **API 개발 규칙**
   - 모든 API는 `/api/` prefix 사용 (예: `/api/auth/token`, `/api/data/upload`)
   - 인증: JWT Bearer Token 방식 (`Authorization: Bearer <token>`)
   - 요청 검증: Django REST Framework Serializer의 `is_valid()` 메서드 사용
   - 응답 형식: DRF 표준 JSON 응답
   - CSV 파싱: Pandas `read_csv()` 사용 후 Django ORM으로 저장
   - 트랜잭션: `transaction.atomic()`으로 데이터 일관성 보장

4. **테스트 실행**
   - Django 표준 테스트 명령어 사용: `cd backend && python manage.py test`
   - Migration 생성: `python manage.py makemigrations`
   - Migration 적용: `python manage.py migrate`
...
2. **각 단계마다 검증** (스스로 생각하기)
   ...
   백엔드 작업의 경우 추가로 확인:
   - Django 앱 구조를 올바르게 따르고 있는가?
   - Serializer로 요청/응답 검증을 했는가?
   - API 엔드포인트가 /api/ prefix를 사용하는가?
   - 필요한 경우 Migration을 생성하고 적용했는가?
   - `python manage.py test`로 테스트가 통과하는가?
   ```
...
1. **완전성 검증** (스스로 생각하기)
   ...
   - 예외 상황을 빠짐없이 처리했는가?
   - 에러 로그가 적절히 기록되는가?
   - Django REST Framework의 예외 처리 방식에 맞게 구현했는가?
...
2. **에러 처리**
   ...
   - Django REST Framework의 예외 처리 (DRF Exception Handler 또는 APIView의 exception_handler)
   - 사용자 친화적 에러 메시지
